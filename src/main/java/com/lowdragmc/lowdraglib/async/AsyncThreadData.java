...
public class AsyncThreadData extends SavedData {
    ...
    public static AsyncThreadData getOrCreate(ServerLevel serverLevel) {
        return serverLevel.getDataStorage().computeIfAbsent(tag -> new AsyncThreadData(serverLevel, tag), () -> new AsyncThreadData(serverLevel), LDLMod.MODID);
    }
    ...
    private void searchingTask() {
        try {
            ...
            e.printStackTrace();
            LDLMod.LOGGER.error("asyncThreadLogic error: {}", e.getMessage());
        } finally {
